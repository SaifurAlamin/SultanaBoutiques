...
export default function Signup() {
  ...
  const [confirmShowPass, setConfirmPass] = useState(false);

  const navigate = useNavigate();
  const from = "/profile#profile";

  localStorage.setItem("user", JSON.stringify(userData));

  // Show Pass
  const handleShowPass = () => setShowPassword((show) => !show);
  const handleConfirmShowPass = () => setConfirmPass((show) => !show);
  ...
  const handleSubmit = async (e) => {
    ...
    const url = "https://www.sultanaboutiques.com/backend/api/signup";
    fetch(url, {
      method: "POST",
      headers: {
        "content-type": "application/json",
                    ...
                    <div className="signin-area">
                      <div className="flex justify-center">
                        <button
                          ...
                          className={`${
                            !agree
                              ? "disabled:opacity-50 text-white bg-rose-600"
