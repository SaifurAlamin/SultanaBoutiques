...
export default function NewarrivalesCard({ sectionTitle }) {
  const [newArraival, setNewArrival] = useState([]);
  ...
  useEffect(() => {
    fetch(
      ...
      .then((res) => res.json())
      .then((data) => setNewArrival(data?.products));
  }, []);
  ...
  return (
    <ViewMoreTitle
      ...
      seeMoreUrl={`/all-products/null/null/is_newarrivals`}
      className="mb-[60px]"
      // state={newArraival}
    >
      <Carousel
      ...
      >
        {newArraival?.slice(0, 10).map((item, i) => (
          <div
          ...
          >
            <Link to={`/single-product/${item?.id}`} state={newArraival}>
              <div
