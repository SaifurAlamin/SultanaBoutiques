...
export default function Drawer({ className, open, action }) {
  ...
  //   error,
  //   isLoading,
  // } = useGetAllWishListQuery(userdata?.id ? userdata?.id : googleUserId);
  ...
  const searchProduct = Habib.filter(
    (p) => p.keyword?.toLowerCase().indexOf(searchInput.toLowerCase()) > -1
  );

  const finalSearch = searchProduct?.map((item) => item?.keyword);

  //flatMap e single single data dey like string
  const mapSearchProduct = finalSearch?.flatMap((item) => item.split(","));

  // aki name  onk keyword thakel seta ke single kore dey ay method
        ...
        >
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex space-x-5 items-center">
                {/* <div className="favorite relative">
                  ...
                  <span className="w-[18px] text-white h-[18px] rounded-full bg-qh2-green absolute -top-2.5 -right-2.5 flex justify-center items-center text-[9px]">
                    {wList?.length}
                  </span>
                </div> */}
              </div>
          ...
          <div className="w-full mt-5 px-5">
            <div className="search-bar w-full h-[34px]  flex ">
              ...
              <div className="w-[50px] h-[40px] rounded-sm bg-rose-600  flex justify-center items-center">
          ...
          <div className="w-full mt-5 px-5 flex items-center space-x-3">
            ...
            <span
              ...
              className={`text-base font-semibold cursor-pointer ${
                tab === "menu" ? "text-rose-500" : "text-qgray "
              }`}
            >
              Main Menu
            </span>
                          ...
                          <div className=" flex justify-between items-center px-5 h-10 bg-white hover:bg-qh2-green transition-all duration-300 ease-in-out cursor-pointer text-qblack hover:text-white">
                            <div className="flex items-center space-x-6">
                              <img
                                ...
                                src={`https://sultanaboutiques.com/backend/${cat?.image}`}
                                alt=""
                              />
              ...
              <ul className="categories-list">
                <li className="category-item">
                  <Link to="/" onClick={action}>
                    <div className=" flex justify-start items-center px-5 h-12 bg-white hover:bg-gray-200 transition-all duration-300 ease-in-out cursor-pointer">
                      <div className="flex items-center space-x-6">
                ...
                {category?.map((item) => (
                  <li className="category-item">
                    <div
                      key={item?.id}
                      className=" flex justify-between items-center px-5 h-12 bg-white hover:bg-gray-200 transition-all duration-300 ease-in-out cursor-pointer"
                    >
                      <Dropdown
                        label={item?.name}
                        size="sm"
                        ...
                        {item?.subcategory?.map((subItem) => (
                          ...
                          >
                            <Dropdown.Item>
                              ...
                              <div
                                key={subItem?.id}
                                className=" flex justify-between items-center px-5 h-12 bg-white  rounded w-full transition-all duration-300 ease-in-out cursor-pointer"
                              >
                                {subItem?.name}
                              </div>
                            </Dropdown.Item>
                ...
                <div className=" flex justify-between items-center px-5 h-12 bg-white hover:bg-gray-200 transition-all duration-300 ease-in-out cursor-pointer">
                  <Dropdown inline label="About" size="sm">
                    <Link to="/about">
                    ...
                    <Link to="/size-guide">
                      <Dropdown.Item>Size Guide</Dropdown.Item>
                    </Link>
                  </Dropdown>
                </div>
              </ul>
